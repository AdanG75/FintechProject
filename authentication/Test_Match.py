...
# list = []
'''
    gqr - Good Quality Regiter           : Fingerprint with the minimum quality to be registered
    ...
    fm  - False Match                    : Pair of diferents fingerprints that is accepted incorrectly
    sr  - Successfully Rejection         : Pair of diferents fingerprints that is rejected correctly
...
def describe_match_results(name_match, fingerprint_match):
    ...
    if name_match == 1 and fingerprint_match == em._MATCH_FINGERPRINT:
        states['sm'] += 1
    elif name_match == 1 and fingerprint_match != em._MATCH_FINGERPRINT:
        states['fr'] += 1
    elif name_match == 0 and fingerprint_match == em._MATCH_FINGERPRINT:
        states['fm'] += 1
    elif name_match == 0 and fingerprint_match != em._MATCH_FINGERPRINT:
        states['sr'] += 1
    else:
...
if __name__ == '__main__':
    print('Beginnig with match test')
    test()