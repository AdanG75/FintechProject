...
class PreprocessingFingerprint(object):
    def __init__(
            self,
            name_fingerprint='figerprint',
            address_output='./fingerprint_process/preprocessing_fingerprints/',
    ...
    def __ridge_segment(self, img):
        ...
        new_rows = np.int(
            self._ridge_segment_blksze * np.ceil((np.float(rows)) / (np.float(self._ridge_segment_blksze))))
        new_cols = np.int(
            self._ridge_segment_blksze * np.ceil((np.float(cols)) / (np.float(self._ridge_segment_blksze))))
        ...
        for i in range(0, new_rows, self._ridge_segment_blksze):
            for j in range(0, new_cols, self._ridge_segment_blksze):
                box = [i, j, min(i + self._ridge_segment_blksze, new_rows),
                       min(j + self._ridge_segment_blksze, new_cols)]
    ...
    def __get_morphology_Mask(self):
        ...
        kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE,
                                          (self._ridge_segment_blksze * 2, self._ridge_segment_blksze * 2))
    ...
    def __save_Fingerprints(self, bin_image, skel_image, roi_image):
        ...
        # cv.waitKey(0)
        is_saved = cv.imwrite(self._address_output + 'normalized_' + self._name_fingerprint + '.bmp', (normin_image))
        if is_saved:
            print(f"Image {'normalized_' + self._name_fingerprint + '.bmp'} was saved")

        is_saved = cv.imwrite(self._address_output + 'roi_' + self._name_fingerprint + '.bmp', (roi_image))
        if is_saved:
            print(f"Image {'roi_' + self._name_fingerprint + '.bmp'} was saved")

        is_saved = cv.imwrite(self._address_output + 'binary_' + self._name_fingerprint + '.bmp', (bin_image))
        if is_saved:
            print(f"Image {'binary_' + self._name_fingerprint + '.bmp'} was saved")

        is_saved = cv.imwrite(self._address_output + 'skeletoned' + self._name_fingerprint + '.bmp', (skel_image))
        if is_saved:
            print(f"Image {'skeletoned' + self._name_fingerprint + '.bmp'} was saved")

    def enhance(self, img, resize=False, ridge_color='white', return_as_image=True, show_fingerprints=False,
